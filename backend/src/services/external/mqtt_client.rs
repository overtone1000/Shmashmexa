...
use crate::services::external::has_mqtt::{HomeAssistantDeviceComponent, HomeAssistantDeviceConfiguration, SWITCH_COMPONENT};
...
mod tests {
    ...
    fn get_device_clear_config()->String
    {
        ...
        cmps.insert(
            "test_component_1".to_string(),
            HomeAssistantDeviceComponent::new_empty(
                SWITCH_COMPONENT,
            )
        );
    ...
    async fn test_publish_device() {
        ...
        MQTTClient::publish_discovery_spawn(&client, mqtt_client.get_discovery_topic(), get_device_config());
    ...
    async fn test_clear_device() {
        let mqtt_client:MQTTClient = MQTTClient::new();
        let (client,mut event_loop)=mqtt_client.initialize().await.expect("Couldn't initialize mqtt client.");
        MQTTClient::publish_discovery_spawn(&client, mqtt_client.get_discovery_topic(), get_device_clear_config());
