export async function photoprism_get_raw(base:string, endpoint:string, key:string, timeout_millis:number):Promise<Response|null>
{
    try{
        ...
        const response = await fetch(request, request_init);
        return response;
    }
    catch (error) {
        console.error(error);
